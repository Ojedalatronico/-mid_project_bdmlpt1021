...
# --- Matches path ---
def general_stats(individual_home,individual_away,total,average,x):
    ...
    return bar_shots(shots,partidovs,x)
...
if seleccion=="General stats":
    ...
    if data=="shots":
        st.pyplot(general_stats("total_shots_home","total_shots_away","total_shots","average_shots",f"{data}"))
    if data=="shots on target":
        st.pyplot(general_stats("shots_on_target_home","shots_on_target_away","total_shots_on_target","average_shots_on_targe",x=data))
    if data=="goals":
    ...
    if data=="red cards":
        st.pyplot(general_stats("red_cards_home","red_cards_away","total_red_cards","average_red_card",data))
